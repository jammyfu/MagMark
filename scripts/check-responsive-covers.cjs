...
(async () => {
  ...
  try {
    ...
    await page.locator('[data-layer="subtitle"] .rc-layer-name').click();
    await page.locator('#rc-subtitle').fill('方图专用副标题');
    await page.locator('[data-layer="title"] .rc-layer-name').click();
    ...
    await page.locator('#mm-responsive-cover details').filter({has: page.locator('#rc-reset')}).locator('summary').click();
