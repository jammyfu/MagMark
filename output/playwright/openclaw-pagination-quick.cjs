...
async function inspectTypography(page) {
  return await page.evaluate(() => {
    const heading = document.querySelector('.page-content h3');
...
async function runScenario(page, scenario) {
  ...
  const pageInfo = await page.evaluate(() => document.querySelector('#page-info')?.textContent || null);
...
async function main() {
  ...
  const scenarios = [
    { name: 'a4-default', format: 'a4', scale: 1 },
    { name: 'mobile-default', format: 'mobile', scale: 1 },
    { name: 'desktop-default', format: 'desktop', scale: 1 },
    { name: 'xiaohongshu-default', format: 'xiaohongshu', scale: 0.75 },
    { name: 'a4-large', format: 'a4', scale: 1, fontSize: 18, lineHeight: 2 },
    { name: 'mobile-large', format: 'mobile', scale: 1, fontSize: 18, lineHeight: 2 },
    { name: 'xiaohongshu-adjusted', format: 'xiaohongshu', scale: 0.75, fontSize: 24, lineHeight: 2.6 },
  ];
